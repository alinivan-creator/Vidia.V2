-- Allow bookings that overlap an existing slot by 5 minutes or less.
-- Matches SLOT_OVERLAP_GRACE_MINUTES in src/utils/datetime.js.

ALTER TABLE public.draft_bookings
  DROP CONSTRAINT IF EXISTS draft_bookings_no_overlapping_slots;
...
ALTER TABLE public.draft_bookings
  ...
  EXCLUDE USING gist (
    business_id WITH =,
    slot_lock_key WITH =,
    tstzrange(selected_slot_start + interval '5 minutes', selected_slot_end, '[)') WITH &&
  )
  WHERE (
