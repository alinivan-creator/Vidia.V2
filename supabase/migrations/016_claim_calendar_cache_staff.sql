-- When claiming a slot, busy rows on the shared calendar (employee_id IS NULL)
-- must block every staff member. An "any specialist" claim must also see
-- per-employee busy blocks. Previously NULL vs UUID never overlapped.
