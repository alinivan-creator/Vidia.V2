-- Atomic slot claim: first writer wins, second gets slot_taken.
-- Exclusion constraint blocks overlapping pending/confirmed rows even if two
-- API requests race past the application-level availability check.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'draft_bookings_no_overlapping_slots'
  ) THEN
    ALTER TABLE public.draft_bookings
...
BEGIN
  ...
  -- Release stale pending locks in this tenant before the overlap check.
